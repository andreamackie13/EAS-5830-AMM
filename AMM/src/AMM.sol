...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		address inToken = sellToken;
		address outToken = (sellToken == tokenA) ? tokenB : tokenA;

		uint256 inReserve = ERC20(inToken).balanceOf(address(this));
		uint256 outReserve = ERC20(outToken).balanceOf(address(this));

		uint256 amountFee = (sellAmount * (1000 - feebps))/1000;

		swapAmt = (outReserve * amountFee) / (inReserve + amountFee);
		require(swapAmt > 0, 'insuficient'); 
		require(swapAmt < outReserve, 'insufficient');
